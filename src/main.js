...
function getOpeningHandNeededRowHeight() {
  // Zones stay one card-row tall so the board matches a compact playmat.
  return OPENING_HAND_ROW_MIN;
}
...
function layoutOpeningHandZones(field) {
  ...
  const rowHeight = getOpeningHandNeededRowHeight();
  const gridMin =
    OPENING_HAND_INSET * 2 + rowHeight * 3 + OPENING_HAND_ZONE_GAP * 2;
  field.style.minHeight = `${gridMin}px`;
...
function getOpeningHandSingleRowStep(field, count) {
  ...
  // Keep one row: compress spacing if needed, but never stack cards fully on top of each other.
